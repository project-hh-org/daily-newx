...
- 너는 매일 예약 실행되는 세션이다. 로컬 파일도, 이전 실행의 기억도 없다. 아래를 **가장 먼저 bash로 직접 조회**한다:
  ...
  2. 리서치 번들 조회: `curl -s -H "Authorization: Bearer $INGEST_TOKEN" "https://daily-newx.vercel.app/api/routines/daily-llm-news-research?date=$(TZ=Asia/Seoul date +%F)"`
  ...
  3. issue_date = 위 1번 날짜, issue_no = 리서치 번들의 `research.issue_no`.
...
2. 리서치 번들 조회 성공 여부(404 재시도 몇 번 했는지), 스윕 A~H 반영 여부(간단히).
